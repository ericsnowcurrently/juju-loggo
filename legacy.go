...
func (f *legacyAdaptingFormatter) Format(rec Record) string {
	return f.legacy.Format(rec.Level, rec.Module, rec.Filename, rec.Line, rec.Timestamp, rec.Message)
}
...
func (shim legacyWriterShim) Write(level Level, module, filename string, line int, timestamp time.Time, message string) {
...
func (law *legacyAdaptingWriter) WriteRecord(rec Record) {
	law.Write(rec.Level, rec.Module, rec.Filename, rec.Line, rec.Timestamp, rec.Message)
}
